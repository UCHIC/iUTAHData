...
class MemoryDatabase(object):
    ...
    def initEditValues(self, seriesID):
        if not self.editLoaded:
            ...
            self.cursor.executemany("INSERT INTO DataValues VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                    self.DataValues)
            self.conn.commit()
    ...
    def initSC(self):
        self.SeriesCatalog = [(s.id, s.site_id, s.site_code, s.site_name, s.variable_id, s.variable_code,
                               s.variable_name, s.speciation, s.variable_units_id, s.variable_units_name,
                               s.sample_medium,
                               s.value_type, s.time_support, s.time_units_id, s.time_units_name, s.data_type,
                               s.general_category,
                               ...
                               s.end_date_time, s.begin_date_time_utc, s.end_date_time_utc, s.value_count) for s in
                              self.series_service.get_all_series()]
        self.cursor.executemany(
            "INSERT INTO SeriesCatalog VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            self.SeriesCatalog)
